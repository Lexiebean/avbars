...
function gvalley_OnEvent()
	...
	if event=="CHAT_MSG_SAY" or event == "CHAT_MSG_BATTLEGROUND" then gvalley_GetTimers() end
	...
	for index,value in ipairs(GAV_Objectives) do
		if string.find(string.lower(arg1),string.lower(value)) then
			if string.find(arg1,GVLocale["UNDERATTACK"]) then
				...
				-- special snowflake graveyard
				local updateSFGY
				if value == GAV_Objectives[8] then
					for i=1,15 do
						local BarText = getglobal("gvalleyT"..i.."y"):GetText()
						if GV_active_timers[i] == 1 and BarText == value then
							if string.find(arg1,GVLocale["HORDE"]) then getglobal("gvalley_T"..i.."_Back"):SetVertexColor(.6,.2,.2,1); GV_Faction[i]="(Horde) " end
							if string.find(arg1,GVLocale["ALLIANCE"]) then getglobal("gvalley_T"..i.."_Back"):SetVertexColor(.2,.2,.6,1); GV_Faction[i]="(Alliance) " end		
							GV_active_timers[i] = 1
							GVTInit[i] = GetTime()
							GVTTarget[i] = 300
							gvalley_main:Show()
					...
					-- list people nearby SFGY so we know who to ignore
					if UnitFactionGroup("player") == "Horde" then
						--factionStr = GVLocale["HORDE"]
					elseif UnitFactionGroup("player") == "Alliance" then
						--factionStr = GVLocale["ALLIANCE"]
					end
					
					if factionStr and string.find(arg1,factionStr) then
						-- somebody from our group is trying to capture SFGY, list all nearby mongs
						local fence = {
							x1 = 44.530,
							y1 = 45.030,
							x2 = 44.885,
						...
						local mongsCount = getn(mongs)
						local message = ""
						for i=1, mongsCount do
							if i == mongsCount then
								message = message..mongs[i]
							else
								message = message..mongs[i]..", "
							end
						end
						
						if mongsCount > 1 then
							DEFAULT_CHAT_FRAME:AddMessage("Those mongs captured Snowfall Graveyard: "..message)
							UIErrorsFrame:AddMessage("Those mongs captured Snowfall Graveyard: "..message, 1.0, 1.0, 0.0, 1.0)
							SendChatMessage("Those mongs captured Snowfall Graveyard: "..message, "BATTLEGROUND")
						elseif mongsCount == 1 then
							local pattenrs = {
								"This mong captured Snowfall Graveyard: %s",
								"%s, next game do NOT capture snowfall",
								"Hero of the day: %s! Captured Snowfall Graveyard.",
								"%s, please do NOT capture that gy next time, it's bad and forces turtle",
							}
							local pattern = pattenrs[random(1,getn(pattenrs))]
							
							DEFAULT_CHAT_FRAME:AddMessage(format(pattern, message))
							UIErrorsFrame:AddMessage(format(pattern, message), 1.0, 1.0, 0.0, 1.0)
							SendChatMessage(format(pattern, message), "BATTLEGROUND")
						end
...
function gvalley_GetTimers()
	if not arg1 or not arg3 or not arg3 == this.language then
		return
	end
	
	local patten = '^%((.-)%) (.+) %- ([0-9]+):([0-9]+) remaining$'
	local found,_, who, objective, minutes, seconds = strfind(arg1, patten)
	...
	__D__._ = _
	__D__.who = who
	__D__.objective = objective
	__D__.minutes = minutes
	__D__.seconds = seconds
	
	local index
	for i=1, getn(GAV_Objectives) do
		if string.find(string.lower(objective), string.lower(GAV_Objectives[i])) then
			index = i
			break
		end
	end
	...
	-- check if we already got the timer
	for i=1,15 do
		local BarText = getglobal("gvalleyT"..i.."y"):GetText()
		if GV_active_timers[i] == 1 and BarText == objective then
			return
		end
	end
	...
	for i=1,15 do
		if GV_active_timers[i]==0 then
			GV_active_timers[i]=1
			getglobal("gvalleyT"..i.."y"):SetText(GAV_Objectives[index])
			if who == GVLocale["HORDE"] then
				getglobal("gvalley_T"..i.."_Back"):SetVertexColor(.6,.2,.2,1); GV_Faction[i]="(Horde) "
			end
			if who == GVLocale["ALLIANCE"] then
				getglobal("gvalley_T"..i.."_Back"):SetVertexColor(.2,.2,.6,1); GV_Faction[i]="(Alliance) "
			end
			GVTInit[i]=GetTime()
			GVTTarget[i]=seconds
			gvalley_main:Show()
